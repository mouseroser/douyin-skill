...
```text
Step 3-6（wemedia）
  → 产出抖音标题 / description / #标签 / 视频路径 / 双封面 / 音乐建议
  ↓
Step 7（晨星确认）
  ↓
Step 7.5（main）
...
| 参数 | 必须 | 说明 |
|------|------|------|
| `--video` | ✅ | 视频文件路径（mp4，建议成片已验收） |
| `--title` | ✅ | 视频标题（≤30字） |
| `--description` | ✅ | 视频描述，正文末尾带 `#标签名` |
| `--vertical-cover` | ✅ | 竖版封面（9:16，2160×3840 或等比） |
| `--horizontal-cover` | ✅ | 横版封面（4:3，1600×1200 或等比） |
| `--music` | ❌ | 背景音乐关键词，默认 `热门` |
| `--topics` | ❌ | 话题标签列表；通常可直接从 description 提取 |
| `--review-timeout` | ❌ | 审核等待超时，默认 30 分钟 |

## 输入前置要求（来自 wemedia Step 6）
...
### 用例 1：wemedia 正式发布链路
1. wemedia 已完成内容创作与 Step 6 抖音适配
2. 晨星在 Step 7 已确认
3. main 调本 skill 执行发布
